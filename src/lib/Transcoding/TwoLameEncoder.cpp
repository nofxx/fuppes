...
{ 
  ...
  if(!CSharedConfig::Shared()->transcodingSettings->TwoLameLibName().empty()) {
    sLibName = CSharedConfig::Shared()->transcodingSettings->TwoLameLibName();
  }  
  ...
  if(!m_TwoLameInit) {        
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_init'");
    return false;
  }
  ...
  if(!m_TwoLameSetNumChannels) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_set_num_channels'");    
    return false;
  } 
  ...
  if(!m_TwoLameSetOutSamplerate) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_set_out_samplerate'");    
    return false;
  } 
  ...
  if(!m_TwoLameSetInSamplerate) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_set_in_samplerate'");    
    return false;
  } 
  ...
  if(!m_TwoLameSetBitrate) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_set_bitrate'");    
    return false;
  }
  ...
  if(!m_TwoLameInitParams) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_init_params'");    
    return false;
  } 
  ...
  if(!m_TwoLameEncodeBufferInterleaved) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_encode_buffer_interleaved'");    
    return false;
  } 
  ...
  if(!m_TwoLameEncodeFlush) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_encode_flush'");    
    return false;
  } 
  ...
  if(!m_TwoLameClose) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_close'");    
    return false;
  } 
  ...
  if(!m_TwoLameGetVersionName) {        
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_get_version_name'");    
  }
  ...
  if(!m_TwoLamePrintConfig) {
    CSharedLog::Log(L_EXT, __FILE__, __LINE__, "cannot load symbol 'twolame_print_config'");    
  }
