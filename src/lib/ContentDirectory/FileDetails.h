...
class ImageItem: public BaseItem
{
  public:    
    ...
    // res
    fuppes_off_t  size() { return m_metadata.size; }
    unsigned int  width() { return m_metadata.width; }
    unsigned int  height() { return m_metadata.height; }
    unsigned int  colorDepth() { return m_metadata.color_depth; }
};
...
class VideoItem: public BaseItem
{
  public:
    ...
    std::string   producer() { return m_metadata.producer; }
    std::string   director() { return m_metadata.director; }

    std::string   language() { return m_metadata.language; }


    // item:videoItem:movie
    std::string   seriesTitle()   { return m_metadata.series_title; }     // e.g. "Seinfeld - Season 3"
    std::string   programTitle()  { return m_metadata.program_title; }    // e.g. "The Pez Dispenser"
    unsigned int  episodeNr()     { return m_metadata.episode_nr; }       // e.g. "14"
    unsigned int  episodeCount()  { return m_metadata.episode_count; }    // e.g. "23"
    ...
    unsigned int  height() { return m_metadata.height; }
    unsigned int  colorDepth() { return m_metadata.color_depth; }  
    unsigned int  videoBitrate() { return m_metadata.video_bitrate; }
    /*
  ...
  unsigned int    audio_sample_frequency;
    */
    ...
    // other
    std::string   audioCodec() { return m_metadata.audio_codec; }
    std::string   videoCodec() { return m_metadata.video_codec; }    
};
...
class CFileDetails
{
  ...
  public:
    ...
    static bool getMusicTrackDetails(std::string p_sFileName, AudioItem* audioItem);
    static bool getImageDetails(std::string p_sFileName, ImageItem* imageItem);
	  static bool getVideoDetails(std::string p_sFileName, VideoItem* videoItem);
